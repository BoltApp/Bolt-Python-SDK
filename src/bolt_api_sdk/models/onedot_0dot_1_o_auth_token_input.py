"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from bolt_api_sdk.types import BaseModel
from bolt_api_sdk.utils import FieldMetadata
from enum import Enum
from typing import Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class OneDot0Dot1OAuthTokenInputGrantType(str, Enum):
    r"""The type of OAuth 2.0 grant being utilized.

    The value will always be `authorization_code` when exchanging an authorization code for an access token.

    """

    AUTHORIZATION_CODE = "authorization_code"


class OneDot0Dot1OAuthTokenInputScope(str, Enum):
    r"""The scope issued to the merchant when receiving an authorization code. Options include `bolt.account.manage`, `bolt.account.view`, `openid`. You can find more information on these options in our [OAuth scope documentation](https://help.bolt.com/developers/references/bolt-oauth/#scopes)."""

    BOLT_ACCOUNT_MANAGE = "bolt.account.manage"
    BOLT_ACCOUNT_VIEW = "bolt.account.view"
    OPENID = "openid"


class OneDot0Dot1OAuthTokenInputTypedDict(TypedDict):
    client_id: str
    r"""Merchant publishable key which can be found in the merchant dashboard"""
    client_secret: str
    r"""Your Bolt API Key."""
    code: str
    r"""Fetched value using OTP value from the Authorization Modal."""
    grant_type: OneDot0Dot1OAuthTokenInputGrantType
    r"""The type of OAuth 2.0 grant being utilized.

    The value will always be `authorization_code` when exchanging an authorization code for an access token.

    """
    scope: OneDot0Dot1OAuthTokenInputScope
    r"""The scope issued to the merchant when receiving an authorization code. Options include `bolt.account.manage`, `bolt.account.view`, `openid`. You can find more information on these options in our [OAuth scope documentation](https://help.bolt.com/developers/references/bolt-oauth/#scopes)."""
    state: NotRequired[str]
    r"""A randomly generated string issued to the merchant when receiving an authorization code used to prevent CSRF attacks"""


class OneDot0Dot1OAuthTokenInput(BaseModel):
    client_id: Annotated[str, FieldMetadata(form=True)]
    r"""Merchant publishable key which can be found in the merchant dashboard"""

    client_secret: Annotated[str, FieldMetadata(form=True)]
    r"""Your Bolt API Key."""

    code: Annotated[str, FieldMetadata(form=True)]
    r"""Fetched value using OTP value from the Authorization Modal."""

    grant_type: Annotated[OneDot0Dot1OAuthTokenInputGrantType, FieldMetadata(form=True)]
    r"""The type of OAuth 2.0 grant being utilized.

    The value will always be `authorization_code` when exchanging an authorization code for an access token.

    """

    scope: Annotated[OneDot0Dot1OAuthTokenInputScope, FieldMetadata(form=True)]
    r"""The scope issued to the merchant when receiving an authorization code. Options include `bolt.account.manage`, `bolt.account.view`, `openid`. You can find more information on these options in our [OAuth scope documentation](https://help.bolt.com/developers/references/bolt-oauth/#scopes)."""

    state: Annotated[Optional[str], FieldMetadata(form=True)] = None
    r"""A randomly generated string issued to the merchant when receiving an authorization code used to prevent CSRF attacks"""
